      ...
      <div className="flex justify-center">
        <div className="grid grid-cols-5 gap-3 gap-x-7">
          {Array.from({ length: 30 }).map((_, index) => (
            <div
              key={index}
              className="w-16 h-8 bg-slate-500 rounded-md flex items-center justify-center text-white cursor-pointer hover:bg-third hover:border-none hover:font-bold"
            >
              {index + 1}
            </div>
          ))}
